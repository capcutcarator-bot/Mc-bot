...
function scheduleReconnect(reason) {
  if (reconnecting) return; // already scheduled — avoid double reconnect from error+end firing together
  reconnecting = true;
  console.log(`[RECONNECT] ${reason || 'disconnected'} — retrying in ${(reconnectDelay / 1000).toFixed(0)}s`);
  clearAfkLoop();
  clearGuardLoop();
...
function createBot() {
  ...
  bot.on('error', (err) => {
    ...
    // Some connection failures (e.g. server offline/restarting) only fire
    // 'error' without a following 'end' — schedule a reconnect ourselves
    // as a safety net; the guard flag prevents a duplicate if 'end' does
    // fire right after.
    scheduleReconnect(`error: ${err.message}`);
  });
  bot.on('end', (reason) => scheduleReconnect(`connection ended (${reason || 'unknown'})`));
}
